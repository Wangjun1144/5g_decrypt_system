...
de_nas_5gs_mm_nas_msg_cont(tvbuff_t *tvb, proto_tree *tree, packet_info *pinfo,
    ...
    char *add_string _U_, int string_len _U_)
{
    uint8_t nested_ext_pd;
    uint8_t nested_second_octet;
    ...
    if (len >= 2) {
        nested_ext_pd = tvb_get_uint8(tvb, offset);
        nested_second_octet = tvb_get_uint8(tvb, offset + 1);
        /*
         * Wireshark's default logic only re-enters nested NAS when the current
         * container context is already plain. In our offline decode flow we
         * still want to unwrap obvious plain nested NAS payloads such as
         * 7E 00 ... (plain 5GMM) or 2E .. .. (5GSM), even when the outer NAS
         * message was integrity-protected.
         */
        if (nested_ext_pd == TGPP_PD_5GMM && (nested_second_octet & 0x0f) == NAS_5GS_PLAIN_NAS_MSG) {
            dissect_nas_5gs(tvb_new_subset_length(tvb, offset, len), pinfo, tree, NULL);
            return len;
        }
        if (nested_ext_pd == TGPP_PD_5GSM) {
            dissect_nas_5gs(tvb_new_subset_length(tvb, offset, len), pinfo, tree, NULL);
            return len;
        }
    }
