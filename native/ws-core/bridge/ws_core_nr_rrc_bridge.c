...
ws_core_ensure_initialized(char **error_utf8)
{
    ...
    if (!epan_init(NULL, NULL, false, &app_data)) {
        *error_utf8 = ws_native_strdup("epan_init failed for ws-core nr-rrc bridge");
        g_mutex_unlock(&ws_core_init_mutex);
        return FALSE;
    }
...
ws_collect_flat_fields(epan_dissect_t *edt, GString *json)
{
    ...
    if (edt == NULL || edt->tree == NULL || json == NULL) {
        return FALSE;
    }
...
static void
ws_append_field_tree_node(proto_node *node, GString *json, gboolean include_offsets)
{
    proto_node *child = NULL;
    field_info *fi;
    ...
    if (child != NULL) {
        while (child != NULL) {
            ...
            first_child = false;
            ws_append_field_tree_node(child, json, include_offsets);
            child = child->next;
        }
    }
    g_string_append_c(json, ']');
...
ws_count_proto_nodes_by_prefix(proto_node *node, const char *prefix)
{
    ...
    if (node->finfo != NULL && node->finfo->hfinfo != NULL &&
        node->finfo->hfinfo->abbrev != NULL &&
        g_str_has_prefix(node->finfo->hfinfo->abbrev, prefix)) {
        count++;
    }
...
static const char *
ws_nr_rrc_message_name_from_key(const char *key)
{
    if (key == NULL) {
        return NULL;
    }
    if (strcmp(key, "nr-rrc.ulInformationTransfer_element") == 0) {
        return "ULInformationTransfer";
    }
    if (strcmp(key, "nr-rrc.rrcSetupComplete_element") == 0) {
        return "RRCSetupComplete";
    }
    if (strcmp(key, "nr-rrc.rrcReconfigurationComplete_element") == 0) {
        return "RRCReconfigurationComplete";
    }
    if (strcmp(key, "nr-rrc.securityModeComplete_element") == 0) {
        return "SecurityModeComplete";
    }
    if (strcmp(key, "nr-rrc.ueCapabilityInformation_element") == 0) {
...
ws_detect_rrc_message_name_from_node(proto_node *node)
{
    ...
    if (node->finfo != NULL && node->finfo->hfinfo != NULL &&
        node->finfo->hfinfo->abbrev != NULL) {
        name = ws_nr_rrc_message_name_from_key(node->finfo->hfinfo->abbrev);
        if (name != NULL) {
            return name;
        }
    }
    ...
    child = node->first_child;
    while (child != NULL) {
        name = ws_detect_rrc_message_name_from_node(child);
        if (name != NULL) {
            return name;
        }
        child = child->next;
...
static gboolean
ws_decode_nr_rrc_payload(const ws_native_nr_rrc_decode_request *request, GString *json, char **error_utf8)
{
    epan_dissect_t *edt;
    tvbuff_t *tvb;
    frame_data fd;
    ...
    const char *message_name = "decoded";
    ...
    edt->tvb = tvb;
    add_new_data_source(&edt->pi, tvb, "NR-RRC UL-DCCH");

    TRY {
        (void) dissect_nr_rrc_UL_DCCH_Message_PDU(tvb, &edt->pi, edt->tree, NULL);
        decoded = TRUE;
    }
    CATCH_ALL {
    ...
    if (decoded) {
        ...
        if (edt->tree != NULL) {
            const char *detected_name = ws_detect_rrc_message_name_from_node(edt->tree);
            if (detected_name != NULL) {
                message_name = detected_name;
            }
        }

        g_string_append(json, "{\"bridgeVersion\":\"ws-core-minimal\",\"protocolName\":\"nr-rrc\",");
        g_string_append(json, "\"pduType\":\"UL_DCCH_Message\",\"messageName\":\"");
        ws_json_append_escaped(json, message_name);
...
int
ws_native_decode_nr_rrc(const ws_native_nr_rrc_decode_request *request, ws_native_decode_result *result)
{
    GString *json;
    ...
    if (!ws_decode_nr_rrc_payload(request, json, &error_utf8)) {
        g_string_free(json, TRUE);
        result->error_utf8 = error_utf8 == NULL ? ws_native_strdup("nr-rrc decode failed") : error_utf8;
        return -1;
    }
