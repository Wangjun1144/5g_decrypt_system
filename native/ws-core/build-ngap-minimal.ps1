param(
    ...
    [string]$GeneratedRoot = "D:\\ideaterm\\5g-decrypt-system\\native\\ws-core\\build-ngap-gen",
    [string]$OutputRoot = "D:\\ideaterm\\5g-decrypt-system\\native\\ws-core\\build-ngap-minimal",
    [string]$OutputName = "ws_core_ngap_minimal.dll",
    [switch]$Clean
)
...
if (-not (Test-Path (Join-Path $GeneratedRoot "packet-ngap.c"))) {
    throw "Generated NGAP source not found at $GeneratedRoot\\packet-ngap.c"
}
...
$commonArgs = @(
    ...
    "-DWS_BUILD_DLL",
    "-DENABLE_STATIC",
    "-DWS_CORE_NGAP_MINIMAL_BUILD=1"
)

$sources = @(
    "$ConfigRoot\\bridge\\ws_core_ngap_bridge.c",
...
Write-Host "Created minimal NGAP bridge: $outputBinary"
